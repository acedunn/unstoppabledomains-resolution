...
export default class Uns extends NamingService {
  ...
  private unsl1: UnsInternal;
  private unsl2: UnsInternal;
  readonly name: NamingServiceName = NamingServiceName.UNS;

  constructor(source?: {locations: {Layer1: UnsSource; Layer2: UnsSource}}) {
    super();
    if (!source) {
      source = {
        locations: {
  ...
  static async autoNetwork(config: {
    ...
    if (
      hasProvider(config.location.Layer1) &&
      hasProvider(config.location.Layer2)
    ) {
      providerLayer1 = config.location.Layer1.provider;
      providerLayer2 = config.location.Layer2.provider;
    } else {
      if (!config.location.Layer1['url'] || !config.location.Layer2['url']) {
      ...
      providerLayer1 = FetchProvider.factory(
        ...
        config.location.Layer1['url'],
      );
      providerLayer2 = FetchProvider.factory(
        NamingServiceName.UNS,
        config.location.Layer2['url'],
      );
    }
    ...
    return new this({
      locations: {
        ...
        Layer2: {network: networkNameLayer1, provider: providerLayer2},
      },
    });
  }
  ...
  async isSupportedDomain(domain: string): Promise<boolean> {
    ...
    return this.unsl1.getExists(this.namehash(domain));
  }
  ...
  async allRecords(domain: string): Promise<CryptoRecords> {
    ...
    return recordsL2 || (await promiseL1);
  }
