...
export default class UnsInternal {
  ...
  constructor(location: UnsLocation, source: UnsSource) {
  ...
  private checkNetworkConfig(location: UnsLocation, source: UnsSource): void {
    if (!source.network) {
  ...
  private async getManyByHash(
    ...
    const [keys, values] = (await this.readerContract('getManyByHash', [
      hashes,
      tokenId,
    ])) as [string[], string[]];
